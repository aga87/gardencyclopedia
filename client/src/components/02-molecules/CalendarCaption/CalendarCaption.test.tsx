...
test('CalendarCaption renders the legend with no visual regression', () => {
  ...
  expect(screen.getByRole('table', { name: /legend/i })).toBeInTheDocument();
  expect(screen.getByRole('columnheader', { name: /sow/i })).toBeInTheDocument();
  expect(screen.getByRole('columnheader', { name: /harvest/i })).toBeInTheDocument();
